...
function chooseCard(card) {
    if(canChooseCard()) {
        ...
        flipCard(card, false)
        // 왜 flip했는데 카드가 뒤집어지는 속도가 이렇게 다르지? 했는데 클릭하고 커서를 안 치우면 계속 뒤집어져 있음ㅡㅡ;;
        // 짜증나게 얘도 클릭하고 포인터를 바로 치우네...ㅠㅠ
        setTimeout(() => {
            flipCards(false)
...
function checkForIncompleteGame() {
    const serialziedGameObj = getLocalStorageItemValue(localStorageGameKey)
    if (serialziedGameObj) {
        gameObj = getObjectFromJSON(serialziedGameObj)
...
function shuffleCards() {
    ...
    function shuffle() {
        ...
        if (shuffleCount == 500) {
            ...
            dealCards()
            updateStatusElement(currentGameStatusElem, "block", primaryColor, "please click the card that you think is the ace card")
        } else {
            shuffleCount++
        }
    }
...
function returnGridAreasMappedToCardPos() {
    ...
    // 아 염병... 이걸 forEach() 안에 놔 뒀으니 카드가 이상한 곳에 출력되지 이 븅신아...
...
function createCard(cardItem) {
    // 근데 굳이 이렇게 해야 하는거야?? 뭔가 체계적인 것처럼 보이는데 사실 코드가 더 길어지지 않나?
    ...
    // add class to from card element
